...
namespace Microsoft.TMSN.IO
{
...
#endif
	{
		...
		{
			if (basePath == null) return subPath;
			if (subPath == null) return basePath;
			if (basePath.Length == 0) return subPath;
			if (subPath.Length == 0) return basePath;
			...
			// check for TMSNStore:
			if (TMSNStoreUtility.IsTMSNStore(subPath))
			{
				return subPath;
			}
			...
			if (TMSNStoreUtility.IsTMSNStore(basePath))
			{
				// is this best?
				return basePath.Substring(0, basePath.IndexOf(':') + 1) +
					PathCombine(TMSNStoreUtility.StorePath(basePath), subPath);
			}
		...
		public static string PathRoot(string path)
		{
			...
			if (nameLower.StartsWith("cosmos:"))
			{
				if (nameLower.Length < "cosmos://".Length)  return "cosmos:";
			...
			{
				// what is right here?
				if (path.Length < "cockpit://aa".Length) return path;
			...
			{
				// what is right here? ***
				if (nameLower.Length < "multi:a".Length) return path;
			...
			{
				// what is right here? ***
				if (nameLower.Length < "filelist:a".Length) return path;
			...
			// TMSNStore:
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				return TMSNStoreUtility.PathRoot(path);
				//return "store:" + PathRoot(TMSNStoreUtility.StorePath(path));
			}
		...
		public static string PathParent(string path)
		{
			...
			if (nameLower.StartsWith("cosmos://"))
			{
				if (nameLower.Length < "cosmos://a/b/c".Length)  return null;
				int s = nameLower.IndexOfAny(pathSeparators, "cosmos://".Length);
				if (s < 0)
				{
					return null;
			...
			if (nameLower.StartsWith("cockpit:"))
			{
				if (nameLower.Length < "cockpit://a/b".Length) return null;
			...
			{
				// what is right here? ***
				if (nameLower.Length < "multi:a/b".Length) return null;
			...
			{
				// what is right here? ***
				if (nameLower.Length < "filelist:a/b".Length) return null;
			...
			if (nameLower.StartsWith("http://") || nameLower.StartsWith("https://"))
			{
				if (nameLower.Length < "http://a/b".Length)  return null;
			...
			if (SqlTextReader.IsSqlTextReader(path))
			{
				...
				if (s == null || s.Length == 0 || s.Length <= path.Length - 1) return null;
				return s;
			}
			...
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				...
				//return null;
				return path.Substring(0, path.IndexOf(':') + 1) + PathParent(TMSNStoreUtility.StorePath(path));
			}

			if (nameLower == "." || nameLower == "./" || nameLower == ".\\")  return "..";

			// filename with no path seperators:
			if (nameLower.IndexOfAny(pathSeparators) < 0)
			{
		...
		public static string GetCanonicalPath(string path)
		{
			...
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				return TMSNStoreUtility.GetCanonicalPath(path);
			}
		...
		private static string ReducePath(string path)
		{
			try
			{
				...
				if (path.Length == 0 || path == ".") return path;
		...
		private static bool PathAncestor(string ancestor, string path2)
		{
			...
			path2 = path2.TrimEnd(pathSeparators);
			if (ancestor.Length > path2.Length) return false;
		...
		/// TMSNStreams will still reslt in different behavior, since the raw file will be
		...
		public static string[] ExpandWildcards(string pattern)
		{
			...
			foreach (string pat in patterns)
			{
				...
				if (pat == "$" || pat == "-" ||
					...
					TMSNStoreUtility.IsTMSNStore(pat) ||
					...
					patLower == "nul" || patLower == "null")
				{
					...
					if (patLower.StartsWith("cosmos:") &&
						(pat.IndexOfAny(wildChars) >= 0 || pat.IndexOf("...") >= 0))
					{
					...
					{
						if (FileExists(pat)) matchList.Add(pat);
					}
				}
							...
							while (dirsLeft.Count != 0)
							{
								...
								try
								{
									...
									if (pathEmpty)
									{
										...
										{
											if (files[i].StartsWith("./") || files[i].StartsWith(".\\")) files[i] = files[i].Substring(2);
										}
									}
...
#else
			return (string[])matchList.ToArray(typeof(string));
#endif
		}
		...
		public static bool DirectoryExists(string path)
		{
			...
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				...
				//return false;
				return DirectoryExists(TMSNStoreUtility.StorePath(path));
			}
			...
			if (archPath == null)
			{
				...
				for (int i = 0; i < segs.Length; i++)
				{
					...
					if (partial.Length == 2 && partial[1] == ':') continue;
					if (Directory.Exists(partial)) continue;
			...
			if (archPath != null)
			{
				...
				inArch = inArch.Trim('\\');
				if (inArch.Length == 0)  return true;
				// Must check directories...
				if (Z7zDecodeStream.Exists(archPath, inArch + "/*"))  return true;
				if (!Z7zDecodeStream.Exists7z && Path.GetExtension(archPath).ToLower() == ".rar")
				{
					if (RarDecodeStream.Exists(archPath, inArch + "/*"))  return true;
				}
		...
		public static void CreateDirectory(string path)
		{
			...
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				throw new IOException("Cannot create directory for TMSNStore");
			}			
		...
		private static string[] DirectoryEntries(string path, bool allowFile, bool allowDirectory)
		{
			...
			if (!allowFile && !allowDirectory)  return new string[0];
			if (path[path.Length - 1] != '\\' && path[path.Length - 1] != '/')
			{
				path = path + "/";
			}
			...
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				return TMSNStoreUtility.DirectoryEntries(path);
			}
			...
			if (archPath != null)
			{
				...
				// check for path in archive.
				inArch = inArch.Trim('\\');
				if (inArch.Length == 0)  return new string[0];
		...
		private static StreamInfo[] DirectoryEntriesInfo(string path, bool allowFile, bool allowDirectory)
		{
			...
			if (path == null || path.Length == 0) return new StreamInfo[0];
			if (!allowFile && !allowDirectory) return new StreamInfo[0];
			if (path[path.Length - 1] != '\\' && path[path.Length - 1] != '/')
			{
			...
			if (TMSNStoreUtility.IsTMSNStore(path))
			{
				return TMSNStoreUtility.DirectoryEntriesInfo(path);
			}
			...
			if (archPath != null)
			{
				...
				// check for path in archive.
				inArch = inArch.Trim('\\');
				if (inArch.Length == 0) return new StreamInfo[0];
		...
		private static StreamInfo[] CockpitDirectoryEntriesInfo(string path, bool allowFile, bool allowDirectory)
		{
			...
			if (htmlDir == null || htmlDir.Length == 0) return new StreamInfo[0];
		...
		public static bool FileExists(string fileName)
		{
			...
			// check for TMSNStore:
			if (TMSNStoreUtility.IsTMSNStore(fileName))
			{
				return TMSNStoreUtility.Exists(fileName);
			}
			...
			if (archPath != null)
			{
				...
				// check for path in archive.
				inArch = inArch.Trim('\\');
				if (inArch.Length == 0)  return false;
				if (Z7zDecodeStream.Exists(archPath, inArch))  return true;
				if (!Z7zDecodeStream.Exists7z && Path.GetExtension(archPath).ToLower() == ".rar")
				{
					if (RarDecodeStream.Exists(archPath, inArch))  return true;
		...
		public static void Delete(string fileName, bool recursive, bool includeCompressedVersions)
		{
			...
			{
				throw new NotSupportedException("HTTP streams cannot be deleted.");
			}
			...
			if (TMSNStoreUtility.IsTMSNStore(fileName))
			{
				throw new NotSupportedException("Cannot delete a TMSNStore.");
			}
		...
		private static void Copy(string source, string destination, bool translateText)
		{
			...
			if (source.Length == 0) throw new ArgumentNullException("source", "The source cannot be empty.");
			if (destination == null) throw new ArgumentNullException("destination", "The destination cannot be null.");
			if (destination.Length == 0) throw new ArgumentNullException("destination", "The destination cannot be empty.");

			string[] files = ExpandWildcards(source);
			if (files.Length == 0) throw new FileNotFoundException("No files match the pattern: " + source);
			...
			else
			{
				...
				else if (TMSNStoreUtility.IsTMSNStore(nameLower))
				{
					// TMSNStore:
					throw new NotSupportedException("Cannot copy to a TMSNStore.");
				}
			...
			if (PathsEqual(source, fullDestination)) return;
		...
		public static void Copy(Stream source, Stream destination)
		{
			...
			if (destination == null) throw new ArgumentException("destination cannot be null", "destination");
			try
			{
		...
		{
			if (destination == null || destination.Length == 0) throw new ArgumentException("destination cannot be empty", "destination");
			...
			for (int i = 0; i < sources.Length; i++)
			{
				if (sources[i] == null || sources[i].Length == 0) throw new ArgumentException("sources[" + i + "] cannot be empty", "sources");
		...
		public static Thread ConsumeBackground(Stream stream)
		{
			...
			Thread consume = new Thread(consumeStart);
		...
		public static string[] ReadLines(TextReader input)
		{
			try
			{
				...
				if (lines.Count > 0 && ((string)lines[lines.Count - 1]).Length == 0)  lines.RemoveAt(lines.Count - 1);
#if DOTNET2
		...
		static readonly uint[] zip_crctab = 
			{
						...
						unsafe
						{
							...
							if (inFile is UnbufferedStream)
							{
								...
								for (int count = inFileU.Read(out buffer); count > 0; count = inFileU.Read(out buffer))
								{
									...
									while (buffer != bEnd)
									{
										crc32 = (crc32 << 8) ^ crctab[(crc32 >> 24) ^ (((uint)(*buffer)) & 0xFF)];
										buffer++;
									}
								}
							}
							else
							{
								...
								for (int count = inFile.Read(buffer, 0, buffer.Length); count > 0; count = inFile.Read(buffer, 0, buffer.Length))
								{
									...
									fixed (byte* bb = buffer)
									{
										...
										while (b != bEnd)
										{
											crc32 = (crc32 << 8) ^ crctab[(crc32 >> 24) ^ (((uint)(*b)) & 0xFF)];
											b++;
										}
									}
								}
						...
						unsafe
						{
							...
							if (inFile is UnbufferedStream)
							{
								...
								for (int count = inFileU.Read(out buffer); count > 0; count = inFileU.Read(out buffer))
								{
									...
									{
										// New checksum value
										crcZip = (crcZip >> 8) ^ zip_crctab[((int)crcZip ^ (*buffer)) & 0xff];
										buffer++;
									}
								}
							}
							else
							{
								...
								for (int count = inFile.Read(buffer, 0, buffer.Length); count > 0; count = inFile.Read(buffer, 0, buffer.Length))
								{
									...
									fixed (byte* bb = buffer)
									{
										...
										{
											// New checksum value
											crcZip = (crcZip >> 8) ^ zip_crctab[((int)crcZip ^ (*b)) & 0xff];
											b++;
										}
									}
								}
	...
	public class StreamInfo : IComparable
	{
		...
		public string Name
		{
			...
			{
				if (path.Length == 0)  return "";
				int end = path[path.Length - 1] == '/' || path[path.Length - 1] == '\\' ?
					path.Length - 1 : path.Length;
				// what if it ends in multiple slashes? ***
				int start = path.LastIndexOfAny(pathSeperators, end - 1);
				if (start < 0)
				{
					start = 0;
				}
		...
		public string CanonicalPath
		{
			get
			{
				...
				{
					canonicalPath = IOUtil.GetCanonicalPath(path);
				}
				return canonicalPath;
			}
		}
	...
	public class ZStreamReader //: StreamReader
	{
		...
		private static bool defaultToLocalEncoding = false;

		private static int BUFFER_SIZE = 32*1024; //-1; //1024*1024; //32768;
		...
		/// </summary>
		static public bool AllowLibraryFallback
		{
		...
		/// </summary>
		static public bool DefaultToLocalEncoding
		{
			get { return defaultToLocalEncoding; }
			set { defaultToLocalEncoding = value; }
		}
		...
		/// </summary>
		static public string FallbackExtension
		{
	...
	public class ZStreamWriter //: StreamWriter
	{
		...
		/// </summary>
		static public readonly Encoding UTF8Lenient = new UTF8Encoding(false, false);
		static private Encoding writeEncoding = UTF8Lenient;
		static private string   writeNewLine = "\n";
		static private int compressionLevel = 1;
		static private bool breakChunksAtLines = true;
		...
		/// </remarks>
		static public bool DefaultLowFragmentation
		{
		...
		/// </summary>
		static public Encoding WriteEncoding
		{
		...
		static public int CompressionLevel
		{
			...
			set { compressionLevel = Math.Max(0, Math.Min(9, value)); }
		}

		/// <summary>
		/// Get or set whether to break at line boundaries when using chunked streams,
		/// such as <see cref="CosmosWriteStream"/> or <see cref="MultiStream"/>. True by default,
		...
		/// </remarks>
		static public bool BreakChunksAtLines
		{
			get { return breakChunksAtLines; }
			set { breakChunksAtLines = value; }
		}
		...
		/// <summary>
		/// Get or set the newline used for new StreamWriters.
		/// </summary>
		static public string WriteNewLine
		{
			get { return writeNewLine; }
			set { writeNewLine = value; }
		}
		...
		/// <exception cref="ArgumentException">fileName is invalid.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public StreamWriter Open(string outFileName)
		{
		...
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public StreamWriter Open(string outFileName, bool append)
		{
		...
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public StreamWriter Open(string outFileName, bool append, bool breakChunksAtLines)
		{
			return Open(outFileName, append, breakChunksAtLines, false);
		}

		static private StreamWriter Open(string outFileName, bool append, bool breakChunksAtLines, bool unbuffered)
		{
			// check for SqlStream - we don't want to open this as a Stream:
			...
			if (size > 0)
			{
				sw = new StreamWriter(s, WriteEncoding, size);
			}
			else
			{
				sw = new StreamWriter(s, WriteEncoding);
			}
		...
		/// <exception cref="ArgumentException">fileName is invalid.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public StreamWriter OpenUnbuffered(string outFileName)
		{
		...
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public StreamWriter OpenUnbuffered(string outFileName, bool append)
		{
		...
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public StreamWriter OpenUnbuffered(string outFileName, bool append, bool breakChunksAtLines)
		{
	...
	{
		private static string fallbackExtension = "";
		private static bool defaultUnbuffered = false;

		private static int BUFFER_SIZE = 32*1024;  //-1;  //64*1024; //32768;
		...
		/// false, by default.
		/// </summary>
		static public bool DefaultUnbuffered
		{
			get { return defaultUnbuffered; }
			set { defaultUnbuffered = value; }
		}
		...
		static public bool AllowLibraryFallback
		{
			get { return allowLibraryFallback; }
			set { allowLibraryFallback = value; }
		}
		private static bool allowLibraryFallback = true;
#else
		static public bool AllowLibraryFallback
		{
			get { return allowLibraryFallback; }
			set { allowLibraryFallback = false; }
		}
		private static bool allowLibraryFallback = false;
#endif
		...
		private static readonly string[] nullStreamNames = new string[]
			{
				"nul",
				"null"
			};
		...
		{
			if (fileName == null || fileName.Length == 0)  return false;
			for (int i = 0; i < nullStreamNames.Length; i++)
			{
				int len = nullStreamNames[i].Length;
				if (string.Compare(fileName, 0, nullStreamNames[i], 0, len, true) == 0)
				{
						...
						fileName[len] == '/')
					{
						...
						if (len == fileName.Length)  return true;
						for (int j = 0; i < nullStreamNames.Length; j++)
						{
							if (fileName.Length - len == nullStreamNames[j].Length &&
								string.Compare(fileName, len, nullStreamNames[j], 0, nullStreamNames[j].Length, true) == 0)
							{
								return true;
							}
						}
		...
		private static readonly string[] consoleStreamNames = new string[]
			{
				"$",
				"-"
			};
		...
		{
			if (fileName == null || fileName.Length == 0)  return false;
			for (int i = 0; i < consoleStreamNames.Length; i++)
			{
				int len = consoleStreamNames[i].Length;
				if (string.Compare(fileName, 0, consoleStreamNames[i], 0, len, true) == 0)
				{
					if (fileName.Length == len)  return true;
						...
						fileName[len] == '/')
					{
						...
						if (len == fileName.Length)  return true;
						for (int j = 0; i < consoleStreamNames.Length; j++)
						{
							if (fileName.Length - len == consoleStreamNames[j].Length &&
								string.Compare(fileName, len, consoleStreamNames[j], 0, consoleStreamNames[j].Length, true) == 0)
							{
								return true;
							}
						}
		...
		public static string[] DecompressionExtensions
		{
			get
			{
				return (string[])decompressionExtensions.Clone();
			}
		}
		...
		public static string[] DecompressionArchiveExtensions
		{
			get
			{
				return (string[])decompressionArchiveExtensions.Clone();
			}
		}
		...
		private static bool gzipFailure = false;
		//private static bool gzip7zaFailure = false;
		private static bool gzip7zFailure = false;
		//private static bool unrarFailure = false;
		//private static bool full7zaFailure = false;
		private static bool full7zFailure = false;
		...
		private static Stream Open(string fileName, bool buffered, bool bufferedFallback, bool async)
		{
			...
			if (fileName.Length == 0) throw new ArgumentException("fileName", "fileName cannot be empty.");
...
#else
				return new BufferedStream(new FixedStream(Console.OpenStandardInput()), 1024);
#endif
			}
			// check for clipboard:
			if (ClipboardReadStream.IsClipboardStream(fileName))
			...
			{
				if (fileName.Length <= "cockpit:".Length) throw new ArgumentException("Invalid cockpit streamname", "fileName");
				...
				int hostStart = 0;
				while (hostStart < fileName.Length && fileName[hostStart] == '\\') hostStart++;
				if (hostStart < fileName.Length - 2)
				{
					int hostEnd = hostStart + 1;
					while (hostEnd < fileName.Length && fileName[hostEnd] != '\\') hostEnd++;
				...
				string cmd = "get";
				if (fileName.EndsWith("\\") || fileName.EndsWith("/")) cmd = "ls";
			...
			if (SqlTextReader.IsSqlTextReader(fileName))
			{
				return new TextReaderStream(new SqlTextReader(fileName));
			}
			...
			try
			{
						...
						case ".gz":
							{
								...
								if (AllowLibraryFallback && s == null)
								{
									...
									if (s == null)
									{
										// SharpZipLib breaks with a larger buffer!!
				...
				if (s == null)
				{
					...
					if (BUFFER_SIZE > 0)
					{
						s = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, BUFFER_SIZE);
					}
					else
					{
	...
	public class ZStreamOut
	{
		...
		private static bool defaultLowFragmentation = true;
		private static bool breakChunksAtLines = false;

		private static int BUFFER_SIZE = 32*1024;  //64*1024;  //-1;  //64*1024; //32768;
		...
		static public bool DefaultLowFragmentation
		{
			get { return defaultLowFragmentation; }
			set { defaultLowFragmentation = value; }
		}
		...
		/// <exception cref="ArgumentException">fileName is invalid.</exception>
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		static public Stream Open(string fileName)
		{
		...
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		static public Stream Open(string fileName, bool append)
		{
		...
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		static public Stream Open(string fileName, bool append, bool breakChunksAtLines)
		{
			return Open(fileName, append, breakChunksAtLines, true);
		}
		...
		/// <exception cref="InvalidOperationException">The utilities needed to open a stream are not available.</exception>
		/// <exception cref="FileNotFoundException">Append is specified, and fileName cannot be found.</exception>
		static private Stream Open(string fileName, bool append, bool breakChunksAtLines, bool buffered)
		{
			if (fileName == null) throw new ArgumentNullException("fileName", "fileName cannot be null.");
			if (fileName.Length == 0) throw new ArgumentException("fileName", "fileName cannot be empty.");
			...
			if (ZStreamIn.IsConsoleStream(fileName))
			{
				...
				LineBufferedStream res = new LineBufferedStream(Console.OpenStandardOutput(), BUFFER_SIZE);
			...
			if (SqlTextReader.IsSqlTextReader(fileName))
			{
				return new TextWriterStream(new SqlTextWriter(fileName));
			}

			// check for TMSNStore:
			if (TMSNStoreUtility.IsTMSNStore(fileName))
			{
				throw new NotSupportedException("Cannot write to a TMSNStore.");
			}
			...
			if (!forceRaw)
			{
				...
				switch (ext)
				{
					case ".gz":
						{
							...
							//bool failed = false;
							if (append) throw new ArgumentException("Cannot append to a gz stream.", "append");
							try
							{
...
#else
#if SHARPZIP
								s = new ICSharpCode.SharpZipLib.GZip.GZipOutputStream(s, 32*1024);
								((ICSharpCode.SharpZipLib.GZip.GZipOutputStream)s).SetLevel(CompressionLevel);
					...
					case ".bz2":
						{
							if (append) throw new ArgumentException("Cannot append to a 7z stream.", "append");
							try
							{
...
//				{
				...
				if (DefaultLowFragmentation)
				{
					if (BUFFER_SIZE > 0)
					{
						s = new LowFragmentationStream(fileName, append, BUFFER_SIZE);
					}
					else
					{
				...
				else
				{
					if (BUFFER_SIZE > 0)
					{
						s = new FileStream(fileName, append ? FileMode.Append : FileMode.Create, FileAccess.Write, FileShare.Read, BUFFER_SIZE);
					}
					else
					{
...
//				{
//#if UNBUFFERED
//					throw new NotSupportedException("Unbuffered IO is not supported for writing.");
//					// assume sequential and not async? ***
//					//if (BUFFER_SIZE > 0)
//					//{
//					//    return UnbufferedStream.Open(outFileName, append ? FileMode.Append : FileMode.Create, FileAccess.Write, FileShare.Read,
//					//        true, false, 4 * BUFFER_SIZE);
//					//}
//					//else
//					//{
//					//    return UnbufferedStream.Open(outFileName, append ? FileMode.Append : FileMode.Create, FileAccess.Write, FileShare.Read,
...
//				}
				...
				//					{
				//						if (append)  throw new ArgumentException("Cannot append to a gz stream.", "append");
				//						s = new ICSharpCode.SharpZipLib.GZip.GZipOutputStream(s);
				//						((ICSharpCode.SharpZipLib.GZip.GZipOutputStream)s).SetLevel(CompressionLevel);
				//					}
				//				}
				//				catch
		...
		/// </para>
		/// </remarks>
		static public Stream OpenUnbuffered(string fileName)
		{
			return OpenUnbuffered(fileName, false);
		}
		...
		/// </para>
		/// </remarks>
		static public Stream OpenUnbuffered(string fileName, bool append)
		{
		...
		/// </para>
		/// </remarks>
		static public Stream OpenUnbuffered(string fileName, bool append, bool breakChunksAtLines)
		{
			return Open(fileName, append, breakChunksAtLines, false);
		}
		...
		IComparable, IEnumerable, IConvertible
	{
		...
		{
			return v.raw == null || v.raw.Length == 0 ? '\0' : v.raw[0];
		}
		/// <summary>
		/// Convert the specified value to a Var.
		/// </summary>
		...
		{
			return v.raw == null || v.raw.Length == 0 ? decimal.Zero : decimal.Parse(v.raw);
		}
		/// <summary>
		/// Convert the specified value to a Var.
		/// </summary>
		...
		{
			return v.raw != null && v.raw != "0" && string.Compare(v.raw, "false", true) != 0;
		}
		/// <summary>
		/// Convert the specified value to a Var.
		/// </summary>
		...
		public static implicit operator Guid(Var v)
		{
			...
			if (v.raw == null || v.raw.Length == 0) return Guid.Empty;
		...
		{
			return v.raw == null || v.raw.Length == 0 ? DateTime.MinValue :
				DateTime.Parse(v.raw, null,
		...
		private static long ParseInt64(string s)
		{
			...
			{
				throw new FormatException("String cannot be converted to integer: " + s);
			}
		}
		...
		private static float ParseSingle(string s)
		{
			...
			{
				throw new FormatException("String cannot be converted to float: " + s);
			}
		}
		private static double ParseDouble(string s)
		{
			...
			{
				throw new FormatException("String cannot be converted to double: " + s);
			}
		}
		...
		public override int GetHashCode()
		{
			...
			return raw == null ? 0 : raw.GetHashCode();
		}
		...
		public bool Equals(Var obj)
		{
			if (raw == null)
			{
				return obj == null || obj.raw == null;
			}
			return Equals(obj.raw);
		}
		/// <summary>Determines whether this instance and a specified value are the same.</summary>
		/// <returns>true if obj is the same as this instance; otherwise, false.</returns>
		/// <param name="obj">the value to compare to</param>
		...
		{
			return string.CompareOrdinal(raw, obj) == 0;
		}
		/// <summary>Determines whether this instance and a specified value are the same.</summary>
		/// <returns>true if obj is the same as this instance; otherwise, false.</returns>
		/// <param name="obj">the value to compare to</param>
		...
		{
			if ((object)v1 == null) return v2 == Guid.Empty;
			return (Guid)v1 == v2;
		}
		/// <summary>Determines whether the specified values are not the same.</summary>
		/// <returns>true if v1 is not the same as v2; otherwise, false.</returns>
		...
		{
			if ((object)v1 == null) return v2 == DateTime.MinValue;
		...
		{
			if (obj == null) return raw == null ? 0 : 1;
			if (obj is Var)
			{
		...
		{
			return string.CompareOrdinal(raw, obj);
		}
		/// <summary>Compares this instance with a specified value.</summary>
		/// <returns>
		/// A 32-bit signed integer indicating the lexical relationship between the two comparands.
		...
		{
			if ((object)v2 == null) return false;
			if ((object)v1 == null) return v2.raw != null;
		...
		public static bool operator >(Var v1, Var v2)
		{
			...
			if ((object)v2 == null) return v1.raw != null;
		...
		public static Var operator -(Var v)
		{
			...
			if (v.raw == null || v.raw.Length == 0) return new Var(v.raw);
			string s = v.raw.Trim();
			if (s.Length != 0)
			{
		...
		{
			if (v == null) return null;
			if (v.raw == null || v.raw.Length == 0 || v.raw == "0") return (Var)1;
			// how to guess the type? ***
			//char?
			// ugly...
			if (v.raw.Length == 1 && (v.raw[0] < '0' || v.raw[0] > '9'))
			{
				return (Var)((char)v + 1);
			}
			// decimal? **
		...
		public static unsafe string Join(string separator, Var[] value, int startIndex, int count)
		{
			...
			if (startIndex < 0)
			{
				throw new ArgumentOutOfRangeException("startIndex", "startIndex out of range: " + startIndex);
			}
			if (count < 0)
			{
			...
			{
				throw new ArgumentOutOfRangeException("startIndex", "startIndex or count out of range: startIndex = " + startIndex + ", count = " + count);
			}
			if (count == 0)
			{
				return "";
			}
			if (separator.Length == 0 && startIndex == 0 && count == value.Length) return Concat(value);
			int length = 0;
			...
			if (value[startIndex] != null) sb.Append((string)value[startIndex]);
			for (int j = startIndex + 1; j <= num2; j++)
			{
				sb.Append(separator);
				if (value[j] != null) sb.Append((string)value[j]);
			}
			return sb.ToString();
		}
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool Contains(string value) { return raw.Contains(value); }
#endif
		...
		public void CopyTo(int sourceIndex, char[] destination, int destinationIndex, int count) { raw.CopyTo(sourceIndex, destination, destinationIndex, count); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool EndsWith(string value) { return raw.EndsWith(value); }
#if DOTNET2
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool EndsWith(string value, StringComparison comparisonType) { return raw.EndsWith(value, comparisonType); }
#endif
#if DOTNET2
		...
		/// <param name="value">A <see cref="System.String"></see> object to compare to. </param>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool EndsWith(string value, bool ignoreCase, System.Globalization.CultureInfo culture) { return raw.EndsWith(value, ignoreCase, culture); }
#endif
#if DOTNET2
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a <see cref="System.StringComparison"></see> value. </exception>
		public bool Equals(string value, StringComparison comparisonType) { return raw.Equals(value, comparisonType); }
#endif
		...
		public CharEnumerator GetEnumerator() { return raw.GetEnumerator(); }
		...
		public int IndexOf(char value) { return raw.IndexOf(value); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int IndexOf(string value) { return raw.IndexOf(value); }
		...
		public int IndexOf(char value, int startIndex) { return raw.IndexOf(value, startIndex); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int IndexOf(string value, int startIndex) { return raw.IndexOf(value, startIndex); }
#if DOTNET2
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int IndexOf(string value, StringComparison comparisonType) { return raw.IndexOf(value, comparisonType); }
#endif
		...
		public int IndexOf(char value, int startIndex, int count) { return raw.IndexOf(value, startIndex, count); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int IndexOf(string value, int startIndex, int count) { return raw.IndexOf(value, startIndex, count); }
#if DOTNET2
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a valid <see cref="System.StringComparison"></see> value.</exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int IndexOf(string value, int startIndex, StringComparison comparisonType) { return raw.IndexOf(value, startIndex, comparisonType); }
#endif
#if DOTNET2
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a valid <see cref="System.StringComparison"></see> value.</exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int IndexOf(string value, int startIndex, int count, StringComparison comparisonType) { return raw.IndexOf(value, startIndex, count, comparisonType); }
#endif
		...
		public int IndexOfAny(char[] anyOf) { return raw.IndexOfAny(anyOf); }
		...
		/// <exception cref="System.ArgumentNullException">anyOf is null. </exception>
		public int IndexOfAny(char[] anyOf, int startIndex) { return raw.IndexOfAny(anyOf, startIndex); }
		...
		/// <exception cref="System.ArgumentNullException">anyOf is null. </exception>
		public int IndexOfAny(char[] anyOf, int startIndex, int count) { return raw.IndexOfAny(anyOf, startIndex, count); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public string Insert(int startIndex, string value) { return raw.Insert(startIndex, value); }
#if DOTNET2
		...
		public bool IsNormalized() { return raw.IsNormalized(); }
#endif
#if DOTNET2
		...
		public bool IsNormalized(NormalizationForm normalizationForm) { return raw.IsNormalized(normalizationForm); }
#endif
		/// <summary>Reports the index position of the last occurrence of a specified Unicode character within this instance.</summary>
		/// <returns>The index position of value if that character is found, or -1 if it is not.</returns>
		/// <param name="value">A Unicode character to seek. </param>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(char value) { return raw.LastIndexOf(value); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(string value) { return raw.LastIndexOf(value); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(char value, int startIndex) { return raw.LastIndexOf(value, startIndex); }
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(string value, int startIndex) { return raw.LastIndexOf(value, startIndex); }
#if DOTNET2
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a valid <see cref="System.StringComparison"></see> value.</exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(string value, StringComparison comparisonType) { return raw.LastIndexOf(value, comparisonType); }
#endif
		...
		public int LastIndexOf(char value, int startIndex, int count) { return raw.LastIndexOf(value, startIndex, count); }
		...
		/// <exception cref="System.ArgumentOutOfRangeException">count or startIndex is negative.-or- count plus startIndex specify a position not within this instance. </exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(string value, int startIndex, int count) { return raw.LastIndexOf(value, startIndex, count); }
#if DOTNET2
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a valid <see cref="System.StringComparison"></see> value.</exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(string value, int startIndex, StringComparison comparisonType) { return raw.LastIndexOf(value, startIndex, comparisonType); }
#endif
#if DOTNET2
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a valid <see cref="System.StringComparison"></see> value.</exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public int LastIndexOf(string value, int startIndex, int count, StringComparison comparisonType) { return raw.LastIndexOf(value, startIndex, count, comparisonType); }
#endif
		...
		/// <param name="anyOf">A Unicode character array containing one or more characters to seek. </param>
		/// <exception cref="System.ArgumentNullException">anyOf is null. </exception>
		public int LastIndexOfAny(char[] anyOf) { return raw.LastIndexOfAny(anyOf); }
		...
		/// <exception cref="System.ArgumentNullException">anyOf is null. </exception>
		public int LastIndexOfAny(char[] anyOf, int startIndex) { return raw.LastIndexOfAny(anyOf, startIndex); }
		...
		/// <exception cref="System.ArgumentOutOfRangeException">count or startIndex is negative.-or- count plus startIndex specify a position not within this instance. </exception>
		/// <exception cref="System.ArgumentNullException">anyOf is null. </exception>
		public int LastIndexOfAny(char[] anyOf, int startIndex, int count) { return raw.LastIndexOfAny(anyOf, startIndex, count); }
#if DOTNET2
		...
		public string Normalize() { return raw.Normalize(); }
#endif
#if DOTNET2
		...
		/// <param name="normalizationForm">A Unicode normalization form. </param>
		public string Normalize(NormalizationForm normalizationForm) { return raw.Normalize(normalizationForm); }
#endif
		...
		public string PadLeft(int totalWidth) { return raw.PadLeft(totalWidth); }
		...
		/// <param name="totalWidth">The number of characters in the resulting string, equal to the number of original characters plus any additional padding characters. </param>
		/// <exception cref="System.ArgumentException">totalWidth is less than zero. </exception>
		public string PadLeft(int totalWidth, char paddingChar) { return raw.PadLeft(totalWidth, paddingChar); }
		...
		/// <param name="totalWidth">The number of characters in the resulting string, equal to the number of original characters plus any additional padding characters. </param>
		/// <exception cref="System.ArgumentException">totalWidth is less than zero. </exception>
		public string PadRight(int totalWidth) { return raw.PadRight(totalWidth); }
		...
		/// <param name="totalWidth">The number of characters in the resulting string, equal to the number of original characters plus any additional padding characters. </param>
		/// <exception cref="System.ArgumentException">totalWidth is less than zero. </exception>
		public string PadRight(int totalWidth, char paddingChar) { return raw.PadRight(totalWidth, paddingChar); }
#if DOTNET2
		...
		public string Remove(int startIndex) { return raw.Remove(startIndex); }
#endif
		...
		public string Remove(int startIndex, int count) { return raw.Remove(startIndex, count); }
		...
		public string Replace(char oldChar, char newChar) { return raw.Replace(oldChar, newChar); }
		...
		public string Replace(string oldValue, string newValue) { return raw.Replace(oldValue, newValue); }
		...
		public string[] Split(params char[] separator) { return raw.Split(separator); }
		...
		public string[] Split(char[] separator, int count) { return raw.Split(separator, count); }
#if DOTNET2
		...
		public string[] Split(char[] separator, StringSplitOptions options) { return raw.Split(separator, options); }
		...
		/// <exception cref="System.ArgumentException">options is not one of the <see cref="System.StringSplitOptions"></see> values.</exception>
		public string[] Split(string[] separator, StringSplitOptions options) { return raw.Split(separator, options); }
		...
		/// <exception cref="System.ArgumentException">options is not one of the <see cref="System.StringSplitOptions"></see> values.</exception>
		/// <exception cref="System.ArgumentOutOfRangeException">count is negative. </exception>
		public string[] Split(char[] separator, int count, StringSplitOptions options) { return raw.Split(separator, count, options); }
		...
		/// <exception cref="System.ArgumentException">options is not one of the <see cref="System.StringSplitOptions"></see> values.</exception>
		/// <exception cref="System.ArgumentOutOfRangeException">count is negative. </exception>
		public string[] Split(string[] separator, int count, StringSplitOptions options) { return raw.Split(separator, count, options); }
		/// <summary>Determines whether the beginning of this instance matches the specified string.</summary>
		/// <returns>true if value matches the beginning of this string; otherwise, false.</returns>
		/// <param name="value">The <see cref="System.String"></see> to compare. </param>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool StartsWith(string value) { return raw.StartsWith(value); }
		...
		/// <exception cref="System.ArgumentException">comparisonType is not a <see cref="System.StringComparison"></see> value.</exception>
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool StartsWith(string value, StringComparison comparisonType) { return raw.StartsWith(value, comparisonType); }
#endif
#if DOTNET2
		...
		/// <exception cref="System.ArgumentNullException">value is null. </exception>
		public bool StartsWith(string value, bool ignoreCase, System.Globalization.CultureInfo culture) { return raw.StartsWith(value, ignoreCase, culture); }
#endif
		...
		/// <exception cref="System.ArgumentOutOfRangeException">startIndex is less than zero or greater than the length of this instance. </exception>
		public string Substring(int startIndex) { return raw.Substring(startIndex); }
		...
		public string Substring(int startIndex, int length) { return raw.Substring(startIndex, length); }
#if DOTNET2
		IEnumerator<char> IEnumerable<char>.GetEnumerator() { return ((IEnumerable<char>)raw).GetEnumerator(); }
#endif
		IEnumerator IEnumerable.GetEnumerator() { return ((IEnumerable)raw).GetEnumerator(); }
		...
		public char[] ToCharArray() { return raw.ToCharArray(); }
		...
		public char[] ToCharArray(int startIndex, int length) { return raw.ToCharArray(startIndex, length); }
		...
		public string ToLower() { return raw.ToLower(); }
		...
		public string ToLower(System.Globalization.CultureInfo culture) { return raw.ToLower(culture); }
#if DOTNET2
		...
		public string ToLowerInvariant() { return raw.ToLowerInvariant(); }
#endif
		...
		public string ToUpper() { return raw.ToUpper(); }
		...
		/// <param name="culture">A <see cref="System.Globalization.CultureInfo"></see> object that supplies culture-specific casing rules. </param>
		/// <exception cref="System.ArgumentNullException">culture is null. </exception>
		public string ToUpper(System.Globalization.CultureInfo culture) { return raw.ToUpper(culture); }
#if DOTNET2
		...
		public string ToUpperInvariant() { return raw.ToUpperInvariant(); }
#endif
		...
		public string Trim() { return raw.Trim(); }
		...
		public string Trim(params char[] trimChars) { return raw.Trim(trimChars); }
		...
		/// <param name="trimChars">An array of Unicode characters to be removed or null. </param>
		public string TrimEnd(params char[] trimChars) { return raw.TrimEnd(trimChars); }
		...
		/// <param name="trimChars">An array of Unicode characters to be removed or null. </param>
		public string TrimStart(params char[] trimChars) { return raw.TrimStart(trimChars); }
		...
		public char this[int index] { get { return raw[index]; } }
	...
	public class NoPreambleEncoding : System.Text.Encoding
	{
		...
		/// </summary>
		public readonly Encoding UTF16 = new NoPreambleEncoding(Encoding.Unicode);
		/// <summary>
		/// A big-endian UTF16 encoding with no preamble.
		/// </summary>
		public readonly Encoding BigEndianUTF16 = new NoPreambleEncoding(Encoding.BigEndianUnicode);
		...
		{
			if (baseEncoding == null) throw new ArgumentNullException("baseEncoding cannot be null.");
		...
		{
			return new NoPreambleEncoding((Encoding)baseEncoding.Clone());
		}
        /// <summary>
		...
		{
			return baseEncoding.GetByteCount(chars, count);
		}
        /// <summary>
        ...
        /// </returns>
        /// <exception cref="T:System.ArgumentOutOfRangeException">charCount or byteCount is less than zero. </exception>
        /// <exception cref="T:System.ArgumentNullException">chars is null.-or- bytes is null. </exception>
        /// <exception cref="T:System.ArgumentException">byteCount is less than the resulting number of bytes. </exception>
        public override unsafe int GetBytes(char* chars, int charCount, byte* bytes, int byteCount)
		{
			return baseEncoding.GetBytes(chars, charCount, bytes, byteCount);
		}
        /// <summary>
		...
		{
			return baseEncoding.GetCharCount(bytes, count);
		}
        /// <summary>
        ...
        /// <exception cref="T:System.ArgumentOutOfRangeException">byteCount or charCount is less than zero. </exception>
        /// <exception cref="T:System.ArgumentException">charCount is less than the resulting number of characters. </exception>
        public override unsafe int GetChars(byte* bytes, int byteCount, char* chars, int charCount)
		{
			return baseEncoding.GetChars(bytes, byteCount, chars, charCount);
		}
        /// <summary>
		...
		{
			return value is NoPreambleEncoding && ((NoPreambleEncoding)value).baseEncoding.Equals(baseEncoding);
		}
        /// <summary>
	...
	/// <summary>
	/// <para>
	/// The Microsoft.TMSN namespace includes classes and functionality developed
	...
	/// </para>
	/// <para>
	/// Microsoft.TMSN.IO contains functionality to help with input and output operations.
