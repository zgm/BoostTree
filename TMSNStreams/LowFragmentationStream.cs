...
namespace Microsoft.TMSN.IO
{
	...
	public class LowFragmentationStream : FileStream
	{
		...
		private static long  minExtension = 50L*1024L*1024L;
		private static long  maxExtension = 2L*1024L*1024L*1024L;
		private static float extensionFactor = 0.5F;
		...
		{
			get { return minExtension; }
			set
			{
				if (value <= 0)  value = 50L*1024L*1024L;
				minExtension = value;
			}
		}
		...
		public static long MaxExtension
		{
			...
			set
			{
				if (value <= 0)  value = 2L*1024L*1024L*1024L;
				maxExtension = value;
			}
		}
		...
		private void Extend()
		{
			...
			long pos = Position;
			long ext = (long)(pos * extensionFactor);
			if (ext < minExtension)
			{
				ext = minExtension;
			}
			else if (ext > maxExtension)
			{
				ext = maxExtension;
			}
			// this calls out to the system:
			//long len = base.Length;
			long len = extendedLength;
			extendedLength = pos + ext;
			if (extendedLength > len)
			{
				try
				{
					// attempt to keep the over-allocation down, at the beginning
					bool useNT = len < minExtension;
					SetAllocation(extendedLength, useNT);
				}
				catch
				{
		...
		public void Flush(bool truncate)
		{
			try
			{
				base.Flush();
...
//		}
		...
		public override long Seek(long offset, SeekOrigin origin)
		{
			switch (origin)
			{
				...
				case SeekOrigin.Current:
					...
					break;
				case SeekOrigin.End:
					offset = (length > 0 ? length : Position) + offset;
					break;
			}
		...
		public override IAsyncResult BeginWrite(byte[] buffer, int offset, int count, AsyncCallback callback, object stateObject)
		{
			...
			if (Position + count >= extendedLength)
			{
				Extend();
			}
...
//		/// </summary>
		...
		public override long Position
		{
			get
			{
				...
				if (position < 0) position = base.Position;
				return position;
			}
			set
			{
